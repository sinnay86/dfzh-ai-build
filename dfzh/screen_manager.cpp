...
namespace Hooks {
    ...
    void ScreenManager::onSDLGetWindowSize() {
        ...
        // AI 翻译完成标志：Worker 写入新翻译后置位，触发本帧重新执行翻译+建纹理
        const bool ai_translation_updated = AITRANSLATOR.hasNewTranslations();

        if (screen_contents_changed_flag || dict_reloaded_flag || ai_translation_updated) {
            processTranslations();
        }
        if (ai_translation_updated) {
            AITRANSLATOR.clearNewTranslationsFlag();
        }
    ...
    void ScreenManager::processTranslations() {
        ...
        for (auto& sentence : detection_results.sentences) {
            ...
            // 3-tier translation: exact match → rulesets fallback → AI (Ollama)
            if (DICTIONARY.tryTranslate(sentence.content, translation, alignment)) {
                ...
                alignment = 'l';
                is_translated = true;
            } else if (AITRANSLATOR.tryTranslate(sentence.content, translation)) {
                // AI Cache 命中：直接使用中文翻译
                // 未命中时 tryTranslate 内部已将文本加入 Pending + Queue，
                // 这帧继续显示英文，Worker 后台翻译完成后下一帧自动更新
                alignment = 'l';
                is_translated = true;
            }
